...
async function getAccessToken() {
  ...
  if (r.status !== 200) {
    const e = new Error('token-issue-failed: status=' + r.status + ' body=' + r.body);
...
async function getAccountSeq(token) {
  ...
  if (r.status !== 200) {
    const e = new Error('accounts-fetch-failed: status=' + r.status + ' body=' + r.body);
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (r.status !== 200) {
      res.status(502).json({ error: 'holdings-fetch-failed', status: r.status, detail: r.body });
      return;
    }
